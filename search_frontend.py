...
pkl_file = open('DL_standart_body/my_DL_body_dict/my_DL_body_dict_DL_body_dict.pkl', 'rb')
...
# Open the pickle file
pkl_file2 = open('DL_standart_title/my_DL_title_dict/DL_title_dict.pkl', 'rb')
...
# def get_posting_iter(index, index_name):
#     words, pls = zip(*index.posting_lists_iter(index_name))
#     return words, pls
...
#
#             for doc_id, doc_tf in list_of_doc:
...
def search():
    ...
    if len(query) == 0:
      return jsonify(res)
    # BEGIN SOLUTION
    query = query_text_to_dict(query)
    if len(query) == 1:
        title = cosine_similarity(query, titleindex, "standart_title_index", DL_title, 100, True)
        res = title

    elif len(query) == 2:
        body = cosine_similarity_for_body(query, bodyindex, "standart_body_index", DL_body, 500, True)
        title = cosine_similarity(query, titleindex, "standart_title_index", DL_title, 100, False)
        res = merge_results_title_body_anchor(title, body, 0.9, 0.1, 400)
    else:
        body = cosine_similarity_for_body(query, bodyindex, "standart_body_index", DL_body, 500, True)
        title = cosine_similarity(query, titleindex, "standart_title_index", DL_title, 100, False)
        res = merge_results_title_body_anchor(title, body, 0.3, 0.7, 400)
...
def search_body():
    ...
    d = cosine_similarity_for_body(query_text_to_dict(query), bodyindex, "standart_body_index", DL_body, 100, True)
    res_temp = d
    res = res_temp
    result = []
    for i in range(len(res)):
...
def search_title():
    ...
    d = cosine_similarity(query_text_to_dict(query), titleindex, "standart_title_index", DL_title, 100, True)
    res_temp = d
    res = res_temp
...
def test_body():
    ...
    with open('queries_train_num_2.json', 'rt') as f:
        queries = json.load(f)
